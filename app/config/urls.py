...
from distribution.urls import api_router as distribution_api_router
from provider.urls import api_router as provider_api_router
...
default_api_router = routers.DefaultRouter(trailing_slash=False)
...
default_api_router.registry.extend(distribution_api_router.registry)
default_api_router.registry.extend(provider_api_router.registry)

urlpatterns = [
    path('admin/', admin.site.urls),
    path('tst/', include('tst.urls')),
    re_path('api/v4/doc/swagger(?P<format>\.json|\.yaml)$', schema_view.without_ui(cache_timeout=0), name='schema-json'),
    ...
    path('api/', include(default_api_router.urls)),
    # path('api/v4/', include(dataset_router.urls)),
